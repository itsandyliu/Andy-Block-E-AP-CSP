...
def main():
    ...
    while True:
        if selection == 1:
            ...
            random_word = random.choice(word_list)
            print(f"Category: Random")
            attempts= play_hangman(random_word)
            total_score += attempts * 15
            ...
            print()
            print(playername + "'s current score: ", total_score)
            print("Previous scores (from highest to lowest):")
            print_scoreboard()
            check = True
            while check:
                temp = input("Enter m to return menu ")
            ...
            update_scoreboard(playername, total_score)
...
def update_scoreboard(playername, score):
...
def print_scoreboard():
    ...
    with open(SCOREBOARD_FILE, "r") as file:
        scores = [(line.strip().split(": ")[0], int(line.strip().split(": ")[1])) for line in file.readlines()]
